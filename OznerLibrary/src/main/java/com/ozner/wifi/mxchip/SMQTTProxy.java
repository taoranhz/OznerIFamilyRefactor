...
public class SMQTTProxy extends XObject {
    private   String TAG = "SMQTTProxy";
    private MQTT mqtt = new MQTT();
    private MQTTImp mqttImp = new MQTTImp();
    private ArrayList<IMQTTListener> listeners = new ArrayList<>();
    ...
    protected void init(String host,String tag) {
        TAG = tag;
        Log.e(TAG, "init: Host:"+host);
        try {
            mqtt.setHost(host);
...
//        mqtt.setWillRetain(true);//若想要在发布“遗嘱”消息时拥有retain选项，则为true
    ...
    public void registerListener(IMQTTListener listener) {
    ...
    public void unregisterListener(IMQTTListener listener) {
        synchronized (listeners) {
    ...
    public void start() {
        connection.connect(new Callback<Void>() {
            ...
            public void onSuccess(Void value) {
                Log.e(TAG, "SMQTT_connect_onSuccess: ");
            }

            @Override
            public void onFailure(Throwable value) {
                Log.e(TAG, "SMQTT_connect_onFailure: ");
    ...
    public void stop() {
        connection.disconnect(new Callback<Void>() {
            @Override
            public void onSuccess(Void value) {
                Log.e(TAG, "SMQTT_disconnect_onSuccess: ");
            }

            @Override
            public void onFailure(Throwable value) {
                Log.e(TAG, "SMQTT_disconnect_onFailure: ");
            }
        });
    }
   ...
   public boolean subscribe(String topic, Callback<byte[]> callback) {
    ...
    class MQTTImp implements ExtendedListener {
        ...
        public void onConnected() {
            connected = true;
            ArrayList<IMQTTListener> list = new ArrayList<>();
            synchronized (listeners) {
                list.addAll(listeners);
            }

            for (IMQTTListener listener : list) {
                listener.onConnected(SMQTTProxy.this);
            }
        }
        ...
        public void onDisconnected() {
            ...
            for (IMQTTListener listener : list) {
                listener.onDisconnected(SMQTTProxy.this);
            }
        }
...
//            }
        ...
        public void onPublish(UTF8Buffer topic, Buffer body, Callback<Callback<Void>> ack) {
            ...
            for (IMQTTListener listener : list) {
                try {
                    listener.onPublish(SMQTTProxy.this, topic.toString(), body.toByteArray());
                } catch (Exception e) {
                    e.printStackTrace();
                    continue;
                }
